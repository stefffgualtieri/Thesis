...
def hiking_optimization_spike(
    ...
    # theta in [0,50]°, slope = tan(theta)
    theta_deg = torch.randint(0, 51, (pop_size, 1), device=device, dtype=torch.int32).float()
    slope = torch.tan(theta_deg * (math.pi / 180.0))
    vel0 = 6.0 * torch.exp(-3.5 * torch.abs(slope + 0.05))  # [pop_size,1]
    vel = vel0.expand(-1, dim).clone()                      # [pop_size,dim]
    
    #Evaluate the initial fintess:
    for i in range(pop_size):
        vector_to_weights(pop[i], layers)
        spike_times = forward_to_spike(model_snn, X_train)
    ...
    for i in range(1, max_iter + 1):
        ...
        for j in range(pop_size):
            ...
            sweep_factor = torch.randint(1, 4, (1,), device=device).float()
            ...
            v_thf = 6.0 * torch.exp(-3.5 * torch.abs(slope_j + 0.05))  # scalare

            #actual_vel = vel + gamma * (best_hiker - sweep_factor * X_ini)
            gamma = torch.rand_like(X_ini)
            vel[j] = vel[j] + gamma * (best_hiker - sweep_factor * X_ini)

            actual_vel = vel[j] + v_thf
        ...
        if elite_fit < fit[worst_idx].item():
